        ...
        >
          ...
          <div
            className={cn(
              ...
              isMobile || hovered === index ? "opacity-100" : "opacity-0"
            )}
          >
            <div className="text-xs md:text-2xl font-medium bg-clip-text text-transparent bg-gradient-to-b from-neutral-50 to-neutral-200">
              {card.name}
            </div>
            <div className="text-sm md:text-2xl font-medium bg-clip-text text-transparent bg-gradient-to-b from-neutral-50 to-neutral-200">
              <div className="flex justify-center items-center gap-2 mb-4">
                {card.socialMediaLinks.map(
                  (l: { name: string; link: string }, index: number) => (
                    <div
                      className="bg-white rounded-3xl hover:scale-150 transition-transform duration-300"
                      key={index}
                    >
                      <Link href={l.link}>
                        <Image
                          src={`/socialmedia2/${l.name}.png`}
                          height={10}
                          width={30}
                          alt={l.name}
                        />
